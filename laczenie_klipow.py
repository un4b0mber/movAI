...
def get_audio_duration(folder_path):
    from pydub import AudioSegment
    import os
    try:
        # Pobierz pierwszy plik audio z folderu
        audio_files = [f for f in os.listdir(folder_path) if f.lower().endswith(('.mp3', '.wav', '.flac'))]
        if not audio_files:
            print(f"Brak plików audio w folderze: {folder_path}")
            return None
        
        first_audio_file = os.path.join(folder_path, audio_files[0])
        print(f"Próba odczytu długości pliku audio: {first_audio_file}")
        audio = AudioSegment.from_file(first_audio_file)
        return audio.duration_seconds
    except Exception as e:
        print(f"Nie udało się odczytać długości pliku audio z folderu {folder_path}: {e}")
        return None
...
def random_video_merge(audio_folder_path, video_files, output_folder, clip_duration, 
    ...
    audio_duration = get_audio_duration(audio_folder_path)
